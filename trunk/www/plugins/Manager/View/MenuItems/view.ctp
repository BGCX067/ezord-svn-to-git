...
<?php echo $this->Form->create('MenuItem', array ('action' => 'view', 'type' => 'file', 'method' => 'post')) ?>
    ...
    <div id="image-view" class="left">
        ...
        <?php else: ?>
            <a href="javascript:void(0)" id="slider" class="left">
                <?php $image_path = $this->Upload->getUploadUrlPath('medium', $cManager['id'], $iid) ?>
    ...
    <div id="item-view" class="right">
        <h3 class="underline"><?php echo $item['MenuItem']['name']; ?></h3>
        ...
        <ul>
            <li>
                ...
                <?php echo $this->Html->link('Image', '/manager/menu_items/images/'.$iid) ?>
            </li>
            <li>
                <?php echo $this->Html->image('custom_price_s.png')?> 
                <?php echo $this->Html->link('Custom price', '/manager/menu_item_prices/price/'.$iid) ?>
            </li>
            <li>
                <?php echo $this->Html->image('edit_s.png')?> 
                <?php echo $this->Html->link('edit', '/manager/menu_items/edit/'.$iid) ?>
            </li>
            <li>
...
$(function() {
    ...
    $('#delete_item').click(function(){
        ...
        if (confirm('Are you sure?')) {
            $.post(BASE_URL+'/manager/menu_items/delete/'+iid, {'iid': iid}, function(data) {
                if (data=='true') {
